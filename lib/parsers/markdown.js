...
function getHighlighter(conf) {
	...
	switch (typeof conf.highlight) {
		case 'string':
			highlighterPath = resolve(conf.highlight);

			if (highlighterPath) {
				highlighter = require(highlighterPath).highlight;

				if (typeof highlighter !== 'function') {
					logger.error('The syntax highlighting module "%s" does not assign a method to exports.highlight. Using the default syntax highlighter.', conf.highlight);
 ...
 * returns the resulting HTML.
 */
exports.getParser = (options) => {
	const parser = (options && options.parser) ? parserNames[options.parser] : parserNames.markdownit;

	return getParseFunction(parser, options);
};
