...
function buildLink(longname, linkText, options) {
	...
	} else if (longname && isComplexTypeExpression(longname) && /\{@.+\}/.test(longname) === false && /^<[\s\S]+>/.test(longname) === false) { // handle complex type expressions that may require multiple links (but skip anything that looks like an inline tag or HTML tag)
...
function useMonospace(tag, text, options) {
	let cleverLinks;
	let monospaceLinks;
	let result;
	...
	if (hasUrlPrefix(text)) {
		...
		cleverLinks = options.templates.cleverLinks;
		monospaceLinks = options.templates.monospaceLinks;
 ...
 * @param {object} options - env variables
...
exports.resolveLinks = (str, options) => {
	...
	function processLink(string, {completeTag, text, tag}) {
		...
		monospace = useMonospace(tag, text, options);

		return string.replace(completeTag, buildLink(target, linkText, {
			linkMap: longnameToUrl,
			monospace: monospace,
			shortenName: options && options.templates && options.templates.useShortNamesInLinks
		}));
	}
...
exports.createLink = doclet => {
	...
	} else if (!containers.includes(doclet.kind) && fakeContainer) { // mistagged version of a doclet that gets its own HTML file
