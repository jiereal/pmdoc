 ...
 * @module tag/dictionary/definitions
 */
...
function parseTypeText(text) {
	const tagType = type.parse(text, false, true);
...
let baseTags = exports.baseTags = {
	...
	default: {
		onTagged(doclet, {value}) {
			const nodeToValue = astnode.nodeToValue;
	...
	requires: {
		...
		onTagged(doclet, {value}) {
			...
			if (inline.isInlineTag(value, 'link\\S*')) {
				requiresName = value;
			} else { // otherwise, assume it's a module
 ...
 *
 * @param {module:tag/dictionary} dictionary
