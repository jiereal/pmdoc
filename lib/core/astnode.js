// TODO: docs
/** @module core/astnode */
const cast = require('utils/cast').cast;
 ...
 *
 * @alias module:core/astnode.isFunction
 ...
 *
 * @alias module:core/astnode.isScope
...
// TODO: docs
exports.addNodeProperties = (node, debugEnabled = true) => {
	const newProperties = {};
	...
	switch (node.type) {
		...
		case Syntax.MethodDefinition:
			...
			if (parent.type === Syntax.ClassExpression) {
				...
				parent.parent.type === Syntax.ExportDefaultDeclaration) { // for the constructor of a module's default export, use a special name
				str = 'module.exports';
			} else if (node.kind === 'constructor' && parent.parent &&
				parent.parent.type === Syntax.ExportNamedDeclaration) { // for the constructor of a module's named export, use the name of the export declaration
				str = nodeToValue(parent.parent);
			} else if (node.kind === 'constructor') { // for other constructors, use the name of the parent class
				str = nodeToValue(parent);
			} else if (parent.parent && parent.parent.type === Syntax.ExportDefaultDeclaration) { // if the method is a member of a module's default export, ignore the name, because it's irrelevant
				str = '';
			} else { // otherwise, use the class's name
...
const getParamNames = exports.getParamNames = node => {
	...
	return node.params.map(param => nodeToValue(param));
};
	...
	switch (node.type) {
		...
		case Syntax.ExportNamedDeclaration:
			...
			if (info.node.declaration) {
				...
				// TODO: This duplicates logic for another node type in `core/visitor` in
