 ...
 * @requires module:tag/dictionary
 * @requires module:tag/validator
 * @requires module:tag/type
 * @requires module:utils/logger
 * @requires module:util
 */
let dictionary = require('tag/dictionary');
const validator = require('tag/validator');
const type = require('tag/type');
...
function addHiddenProperty(obj, propName, propValue) {
	Object.defineProperty(obj, propName, {
		value: propValue,
		writable: true,
		enumerable: Boolean(global.debug),
		configurable: true
	});
}
...
function parseType({text, originalTitle}, {canHaveName, canHaveType}, meta) {
	try {
		...
		console.log(require('chalk').red(e));
		logger.error(
 ...
 * {@link module:core/doclet._replaceDictionary}, which also updates this module's tag dictionary.
 *
 * @private
...
class Tag {
	...
	constructor(tagTitle, tagBody, meta) {
		...
		this.title = dictionary.normalise(this.originalTitle);
		...
		validator.validate(this, tagDef, meta);
	}
}
