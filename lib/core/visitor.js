...
const Doclet = require('core/doclet');
 ...
 *
 * @private
 * @param {module:core/parser.Parser} parser - The JSDoc parser.
...
function makeConstructorFinisher(parser) {
	return e => {
		...
		} else {	// otherwise, we want the ClassDeclaration
		...
		combined = Doclet.combine(doclet, parentDoclet);
...
function trackVars(parser, {enclosingScope}, {code, finishers}) {
	...
	if (enclosingScopeId) {
		...
		doclet = parser._getDocletByLongname(name.LONGNAMES.GLOBAL);
	}
...
function makeSymbolFoundEvent(node, parser, filename) {
	...
	switch (node.type) {
		...
		case Syntax.AssignmentExpression:
			...
			basename = name.getBasename(e.code.name);
			if (basename !== 'this') {
...
class Visitor {
	 ...
	 *
	 * @param {module:core/parser.Parser} parser - The parser instance.
	 */
	setParser(parser) {
		this._parser = parser;
	}
