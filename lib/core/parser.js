...
// TODO: docs
exports.createParser = (type, options) => {
	let modulePath;
	...
	return new (require(modulePath).Parser)(null, null, null, options);
};
...
// TODO: docs
/**
 * @alias module:core/parser.Parser
...
class Parser extends EventEmitter {
	 ...
	 * @param walkerInstance
	 * @param parserOptions
	 * @returns {*}
	 */
	constructor(builderInstance, visitorInstance, walkerInstance, parserOptions) {
		super();

		this.clear();

		this._astBuilder = builderInstance || new (require('core/astbuilder').AstBuilder)(parserOptions);
		this._visitor = visitorInstance || new (require('core/visitor').Visitor)();
		this._walker = walkerInstance || new (require('core/walker').Walker)();
	 ...
	 *
	 * @fires module:core/parser.Parser.parseBegin
	 * @fires module:core/parser.Parser.fileBegin
	 * @fires module:core/parser.Parser.jsdocCommentFound
	 * @fires module:core/parser.Parser.symbolFound
	 * @fires module:core/parser.Parser.newDoclet
	 * @fires module:core/parser.Parser.fileComplete
	 * @fires module:core/parser.Parser.parseComplete
	 *
	 * @example <caption>Parse two source files.</caption>
	 * var myFiles = ['file1.js', 'file2.js'];
	 * var docs = jsdocParser.parse(myFiles);
	 */
	parse(sourceFiles, encoding = 'utf8') {
		let filename = '';
		let sourceCode = '';
		let sourceFile;
		const parsedFiles = [];
		...
		/**
		 * @event module:core/parser.Parser.parseBegin
		 */
		...
		for (let i = 0, l = sourceFiles.length; i < l; i++) {
			...
			if (sourceFile.indexOf(SCHEMA) === 0) { // 类似 javascript:var a =1;
	...
	// TODO: update docs
	/**
	 * @param {module:core/doclet.Doclet} doclet The parse result to add to the result buffer.
	 */
	addResult(doclet) {
	...
	/** @private */
	_parseSourceCode(sourceCode, filename) {
		let ast;
		let e = {
			filename: filename
		};
		...
		if (!e.defaultPrevented) {
			...
			ast = this._astBuilder.build(sourceCode, filename);
			if (ast) {
				this._walkAst(ast, this._visitor, filename);
			}
		}
	...
	addDocletRef(e) {
		...
		if (e && e.code && e.code.node) {
			...
			} else if ( // keep references to undocumented anonymous functions, too, as they might have scoped vars
	 ...
	 * @return {module:core/doclet.Doclet?} The most recent doclet for the longname.
	 */
	 ...
	 * @return {module:core/doclet.Doclet?} The doclet for the lowest-level class in the node's scope
	 * chain.
	 */
	...
	resolveThis(node) {
		...
		if (node.type !== Syntax.Property && node.enclosingScope) {
			...
			} else { // Otherwise, if there's an enclosing scope, we use the enclosing scope to resolve `this`.
				doclet = this._getDocletById(node.enclosingScope.nodeId);
			}
			...
			if (!doclet) {
				...
				} else { // like: function notAClass(n) { /** global this */ this.name = n; }
					result = doclet.longname;
				}
			} else if (doclet.kind === 'member' && astnode.isAssignment(node)) { // like: var foo = function(n) { /** blah */ this.bar = n; }
				result = doclet.longname;
			} else if (doclet.kind === 'class' || doclet.kind === 'module') { // walk up to the closest class we can find
				result = doclet.longname;
			} else if (node.enclosingScope) {
				result = this.resolveThis(node.enclosingScope);
			}
		} else { // For object properties, we use the node's parent (the object) instead.
			doclet = this._getDocletById(node.parent.nodeId);
	 ...
	 * @return {Array.<module:core/doclet.Doclet>} An array of doclets for the parent object or objects, or
 ...
 * @memberof module:core/parser.Parser
 * @type {Object}
