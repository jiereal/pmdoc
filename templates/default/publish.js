...
function needsSignature({kind, type, meta}) {
	...
	if (kind === 'function' || kind === 'class') {
		needsSig = true;
	} else if (kind === 'typedef' && type && type.names && type.names.length) { // typedefs that contain functions get a signature, too
	...
	} else if (kind === 'namespace' && meta && meta.code && meta.code.type && meta.code.type.match(/[Ff]unction/)) { // and namespaces that are functions get a signature (but finding them is a bit messy)
		needsSig = true;
	}
 ...
 */
exports.publish = (taffyData, options, tutorials) => {
	let outdir = path.normalize(options.destination);
	...
	conf = options.templates || {};
	conf.default = conf.default || {};

	templatePath = path.normalize(options.template);
	...
	data = helper.prune(data, options);
	...
	if (conf.default.staticFiles) {
		...
		staticFilePaths.forEach(filePath => {
			...
			filePath = path.resolve(options.pwd, filePath);
	...
	if (outputSourceFiles) {
		generateSourceFiles(sourceFiles, options.encoding);
	}
	...
	function generateTutorial(title, tutorial, filename) {
		...
		html = helper.resolveLinks(html, options); // turn {@link foo} into <a href="foodoc.html">foo</a>
